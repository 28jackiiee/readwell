...
struct AppConstants {
    ...
    // PDF Export
    struct PDF {
        static let defaultFilename = "ReadWell_Report"
...
extension Color {
    static func emotion(_ colorName: String) -> Color {
        // First check if it's a predefined color name
        if let predefinedColor = AppConstants.Colors.emotionColors[colorName] {
            return predefinedColor
        }
        
        // If it starts with #, treat it as a hex color
        if colorName.hasPrefix("#") {
            return Color(hex: colorName)
        }
        
        // If it looks like a hex color without #, add # and try again
        if colorName.count == 6 && colorName.allSatisfy({ $0.isHexDigit }) {
            return Color(hex: "#" + colorName)
        }
